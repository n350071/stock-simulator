class Strategies::AntiLehman
  attr_accessor :report, :up_per

  LehmanShockMonthIDs = [158, 157, 156, 155, 154, 153, 152, 151, 150, 149, 148, 147, 146, 145, 144, 143, 142, 141, 140, 139, 138, 137, 136, 135, 134, 133, 132]
  ...
  # リーマンの期間になったら、すべてを売る
  def run
    if LehmanShockMonthIDs.include?(report.month.id)
      sell_all
    else
      buy
      sell
  ...
  def sell_all
    report.report_tickers.each{ |rt|
      ...
      report.sell(target_mticker, target_mticker.close)
      puts "sell_all target_ticker: #{target_mticker.ticker.id}, price: #{target_mticker.open}, cash: #{report.cash}, tickers: #{report.report_tickers.map(&:ticker).map(&:id)}"
  }
  end
